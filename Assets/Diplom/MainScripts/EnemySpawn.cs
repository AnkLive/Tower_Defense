...
public class EnemySpawn : MonoBehaviour
{
    ...
    [field: SerializeField, Header("Точка спавно врагов")]
    private GameObject _spawn { get; set; }
    [field: SerializeField, Header("Ссылка на редактор волн врагов")]
    private WaveManager _waveManager { get; set; }
    ...
    {
        StartCoroutine(SpawnEnemy(_enemyListTimeBetween[_numberOWave]));
    }

    private void TimeBetweenWaves()
    {
        if (timeStamp <= Time.time && _startNextWave)
        {
            StartCoroutine();
            _startNextWave = false;
            timeStamp = Time.time + _waveManager._timeBetweenWaves;
        }
    }
